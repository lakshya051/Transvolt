...
#         stats = {
#             'min': float(self.df['Values'].min()),
#             'max': float(self.df['Values'].max()),
#             'mean': float(self.df['Values'].mean()),
#             'median': float(self.df['Values'].median()),
#             'std': float(self.df['Values'].std()),
...
#     def _convert_to_json_serializable(self, row):
...
#         results = {
#             'statistics': stats,
#             'peaks': [self._convert_to_json_serializable(row) for row in peak_df.head(20).to_dict('records')],
#             'lows': [self._convert_to_json_serializable(row) for row in low_df.head(20).to_dict('records')],
#             'below_20': [self._convert_to_json_serializable(row) for row in below_20.head(20).to_dict('records')],
#             'accelerating_down': [self._convert_to_json_serializable(row) for row in accel_down.head(20).to_dict('records')],
#             'plots': plots,
#             'data_shape': [int(self.df.shape[0]), int(self.df.shape[1])]
#         }
...
def generate_sample_data():
    ...
    # Generate timestamps (every hour for a year)
    timestamps = []
    current = start_date
    while current <= end_date:
        timestamps.append(current.strftime('%d-%m-%Y %H:%M'))
        current += timedelta(hours=1)
    
    # Generate realistic-looking data with trends, peaks, and noise
    np.random.seed(42)  # For reproducible data
    n = len(timestamps)
    
    # Base trend
    base_trend = 50 + 30 * np.sin(np.linspace(0, 4*np.pi, n)) + 10 * np.sin(np.linspace(0, 20*np.pi, n))
    
    # Add seasonal variations
    seasonal = 15 * np.sin(np.linspace(0, 2*np.pi, n))
    
    # Add random noise
    noise = np.random.normal(0, 5, n)
    
    # Add some dramatic drops and peaks
    values = base_trend + seasonal + noise
    
    # Add some specific features for testing
    for i in range(0, n, 1000):  # Every 1000 points
        if i < n:
            values[i:i+10] = values[i] - 20  # Create drops
    
    for i in range(500, n, 1500):  # Create some peaks
        if i < n:
            values[i:i+5] = values[i] + 25
    
    # Ensure some values are below 20 for testing
    low_indices = np.random.choice(n, size=50, replace=False)
    values[low_indices] = np.random.uniform(5, 19, 50)
...
@app.route('/analyze_sample')
def analyze_sample():
    """Analyze automatically generated sample data"""
    try:
        if analyzer.load_sample_data():
            ...
            if results:
                ...
                return jsonify({'error': 'Failed to analyze sample data'}), 500
        else:
...
def download_sample():
    """Generate and return sample CSV data for download"""
    try:
        ...
        return jsonify({'error': f'Error generating sample CSV: {str(e)}'}), 500
...
<body>
    <div class="container">
        ...
        <div class="upload-section">
            <h3>Upload CSV File</h3>
            <div class="info-box">
                <strong>CSV Format:</strong> File should have 'Timestamp' and 'Values' columns<br>
                <strong>Timestamp format:</strong> DD-MM-YYYY HH:MM (e.g., 01-01-2024 12:00)
            </div>
            <input type="file" id="csvFile" accept=".csv" class="file-input">
            <br>
    ...
    <script>
        ...
        function createTable(data, columns) {
            ...
            if (data.length > 10) {
                html += `<p><em>Showing first 10 of ${data.length} results</em></p>`;
            }
...
if __name__ == '__main__':
    ...
    print("Starting Data Analysis Dashboard...")
    print("Features:")
    print("✓ Upload CSV files with 'Timestamp' and 'Values' columns")
    print("✓ Use automatically generated sample data for testing")
    print("✓ Download sample CSV file for reference")
